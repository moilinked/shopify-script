    ...
    payload = {
        ...
        "variables": {
            "definition": {
                ...
                **({"validations": definition["validations"]} if "validations" in definition else {}),
            }
        }
    }
...
for defn in METAFIELD_DEFINITIONS:
    result = create_metafield_definition(defn)
    ...
    if created and not errors:
        print(f"✅ 创建成功: {defn['namespace']}.{defn['key']}")
        success.append(defn["key"])
    else:
        print(f"❌ 创建失败: {defn['namespace']}.{defn['key']} -> {errors}")
        failed.append({"key": defn["key"], "errors": errors})

print(f"\n完成！成功: {len(success)} 个，失败: {len(failed)} 个")
if failed:
    print("失败列表:", json.dumps(failed, ensure_ascii=False, indent=2))
